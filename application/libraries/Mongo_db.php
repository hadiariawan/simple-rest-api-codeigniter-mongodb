...
class Mongo_db {
	...
	*/
	...
	public function __construct($params = array())
	{
        ...
        // choosing config file cin!
        if(!empty($params['config_file'])) $this->config_file = $params['config_file'];
	...
	*/
	 ...
	 public function get($collection = "")
	 {
		...
		// hz edit, allow to choose different type
	 	if ($this->CI->config->item('mongo_return_head') == 'object')
		{
	...
	*/
    ...
    public function mongo_code($code)
    {
        return new MongoCode($code);
    }

    public function mongo_execute($code)
    {
        return $this->db->execute($code);
    }

    public function distinct($collection = "",$fieldname='',$where=array())
     {
        if (empty($collection))
        {
            show_error("In order to retreive documents from MongoDB, a collection name must be passed", 500);
        ...
        $documents = $this->db->{$collection}->distinct($fieldname,$where);
        return $documents;
        // // Clear
        // $this->_clear();
